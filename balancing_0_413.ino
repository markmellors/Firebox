...
// define analogue speed control input pin, requires 0-5v eg from a pot
#define POT_APIN   3
...
void setLeftMotorSpeed( signed char cNewMotorSpeed )
{
  unsigned char cSpeedVal_Motor1 = 0;
  // Check for full stop command
  if( cNewMotorSpeed == 0 )
...
void setRightMotorSpeed( signed char cNewMotorSpeed )
{
  unsigned char cSpeedVal_Motor2 = 0;
  // Check for full stop command
  if( cNewMotorSpeed == 0 )
  ...
  // Calculate the speed value for motor 2
  if( cNewMotorSpeed >= 100 )
  {
    cSpeedVal_Motor2 = SABER_MOTOR2_FULL_FORWARD;
  ...
  {
    // Calc motor 2 speed (Final value ranges from 1 to 127)
    cSpeedVal_Motor2 = map( cNewMotorSpeed, 
                           -100, 
                            100, 
                            SABER_MOTOR2_FULL_REVERSE,
